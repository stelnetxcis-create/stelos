...
/**
 * Compact month grid for picking a single date.
 *
 * Deliberately dumb: it reports a date and knows nothing about events, so the
 * event sidebar can reuse it for "when does this happen" and for rescheduling
 * without either path growing its own calendar.
 */
Item {
    id: root
    ...
    readonly property var weekdays: H.weekdayLabels(root.firstDayOfWeek, Config.options.calendar.locale, Locale.NarrowFormat)
    ...
    function open(date, titleText) {
        ...
        const start = date ? H.startOfDay(date) : H.startOfDay(DateTime.clock.date);
        root.selected = start;
            ...
            Grid {
                ...
                Repeater {
                    ...
                    delegate: Item {
                        ...
                        readonly property bool isSelected: H.sameDate(dayCell.modelData.date, root.selected)
